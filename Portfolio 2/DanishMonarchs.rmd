...
separator: My dataset is separated by commas
...
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object: The `read_csv()` function is a tidyverse function. I will use it to read the data into a `kings` object.

2. What is the result of running `class()` on the `kings` object created with a tidyverse function: The result of running `class(kings)` on the `kings` object created with a tidyverse function is "spec_tbl_df", "tbl_df", "tbl", "data.frame". This indicates that the object is a tibble, which is a modern version of a data frame in R.

3. How many columns does the object have when created with these different functions?: The object has 11 (13 with the "duration" and "midyear" column which I added in the later objectives) columns when created with the `read_csv()` function, which is the tidyverse function. The other functions may have different numbers of columns depending on how they interpret the data and handle separators.
...
Create a new column called  `duration` in the kings dataset, utilizing the `mutate()` function from tidyverse. Check with your group to brainstorm the options: as seen further down in the code chunk, you can either coerce your data to numbers with `as.numeric()` or you can use `lubridate` package to calculate the duration of reign in years, months and days. I used the first option
...
select(kings_longer_than_average, Name, duration)
...
kings_longest_rulers <- kings %>%
  ...
  slice(1:3) %>%
  mutate(Days = duration * 365 + ifelse(duration %% 4 == 0, 1, 0))

select(kings_longest_rulers, Name, duration, Days)

view(kings_longest_rulers)
```
...
ggplot(kings %>% 
      filter(!is.na(duration) & !is.na(midyear)), 
